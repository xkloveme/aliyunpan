...
export default defineComponent({
  setup() {
    ...
    //   if (useUserStore().user_id) return UserDAL.GetUserList()
    ...
    return {
        handleUserChange, handleRefreshUserInfo, handleSign, handleLogOff, handleLogin, userList, userStore,
        handleUserSpace
      }
  }
})
    ...
    <template #content>
      <div v-if="userStore.userLogined" style="width: 300px">
        <a-row justify="center" align="stretch">
          <a-col flex="150px">
            <span class="username">Hi {{ userStore.GetUserToken.nick_name ? userStore.GetUserToken.nick_name :
              userStore.GetUserToken.user_name }}</span>
          </a-col>
          <a-col flex="auto"></a-col>
          <a-col flex="50px">
            <a-button type="text" size="small" tabindex="-1" style="min-width: 46px; padding: 0 8px" title="刷新账号信息"
              @click="handleRefreshUserInfo">刷新</a-button>
          </a-col>
          <a-col flex="none">
          ...
          <a-col flex="none">
            <a-button type="text" size="small" tabindex="-1" style="min-width: 46px; padding: 0 8px" title="退出该账号"
              @click="handleLogOff"> 退出 </a-button>
          </a-col>
        </a-row>
        ...
        <a-row justify="center" align="stretch">
          ...
          <a-col flex="none">
            <a-button type="text" size="small" tabindex="-1" style="min-width: 46px; padding: 0 8px" status="warning"
              @click="handleUserSpace">容量详情</a-button>
          </a-col>
        </a-row>
        ...
        <a-list style="margin: 12px 0 24px 0" :max-height="300" size="small" :data="userList" class="userlist"
          ...
          <template #item="{ item, index }">
            <a-list-item :key="index">
              ...
              <span :title="item.user_name" style="max-width:172px;display: :inline-block;">{{ item.nick_name ?
                item.nick_name : item.user_name }}</span>
              <a-switch size="small" :model-value="userStore.user_id == item.user_id" title="切换到这个账号" tabindex="-1"
                @change="(val: any) => handleUserChange(val as any, item.user_id)">
